...
export default function exaWebsearchExtension(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "Use operation='search' by default. Prefer includeHighlights=true for agent workflows because highlights are token-efficient.",
      "Use operation='contents' when you already have URLs and need page text, summaries, links, subpages, or highlights.",
      "Use includeText=true only when the task needs fuller page context; cap text with maxTextCharacters.",
      "Use maxAgeHours=0 only when freshness is critical; omit maxAgeHours for normal searches.",
      ...
      "Do not use deprecated useAutoprompt or livecrawl options.",
    ],
